...
//        }
        ...
        WriteCommandAction.runWriteCommandAction(project,()->{
            editor.getDocument().replaceString(selectionModel.getSelectionStart(),selectionModel.getSelectionEnd(),translate);
        });
    ...
    public String translate(List<String> text){
        ...
        for (int i = 0; i < text.size(); i++) {
            List<Map<String, Object>> mapList = SqliteFactory.getInstance().getBaseDb().queryForList("select * from fanyi where word_zh = ? limit 1", text.get(i));
            if (mapList.isEmpty()){
                continue;
            }
